...
public final class QuarkusApplicationRemoteDevDeployments {
    ...
    public static AcquiredHandle getOrStart(DeploymentRegistry registry, String id, Parameters parameters) {
        try {
            DeploymentHandle existing = registry.get(id, DeploymentHandle.class);
            if (existing != null) {
                ...
                return new AcquiredHandle(false, id);
            }
            registry.start(id, DeploymentRegistry.ChangeBehavior.NONE,
                    QuarkusApplicationRemoteDevDeploymentHandle.class, parameters.closeReceiptFile(), id);
            return new AcquiredHandle(true, id);
    ...
    public record Parameters(Path closeReceiptFile) {
    }

    public record AcquiredHandle(boolean started, String id) {
        public RemoteDevPackageClient client() {
            return QuarkusApplicationRemoteDevClients.get(id);
        }

        public void closeClient() {
            QuarkusApplicationRemoteDevClients.close(id);
        }

        public void replaceClient(RemoteDevPackageClient client) {
            QuarkusApplicationRemoteDevClients.replace(id, client);
        }
    }
}
