...
public final class QuarkusApplicationDevDeployments {
    ...
    public static AcquiredHandle getOrStart(DeploymentRegistry registry, String id, Parameters parameters) {
        try {
            ...
            if (existing != null) {
                if (!existing.configFingerprint().equals(parameters.configFingerprint())) {
                    throw new GradleException("A Quarkus dev session is already running for this task with different "
                            + "configuration. Stop the current continuous build and restart quarkusApplicationDev.");
                }
                if (!existing.isRunning()) {
                    throw new GradleException("A Quarkus dev session exists for this task but is not running. Stop the "
                            + "current continuous build and restart quarkusApplicationDev.");
                }
            ...
            QuarkusApplicationDevDeploymentHandle started = registry.start(id, DeploymentRegistry.ChangeBehavior.NONE,
                    ...
                    parameters.launchParameters(), parameters.closeReceiptFile());
    ...
    public static String configFingerprint(GradleNativeDevModeLauncher.Parameters parameters) {
        MessageDigest digest = sha256();
        ...
        sortedPaths(parameters.devModeClasspath()).forEach(path -> update(digest, path));
